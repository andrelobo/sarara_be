...
const buildCommandFilter = ({ status, tableId, waiterId, currentUser }) => {
  const filter = {};
...
const addItemToCommand = async ({ commandId, productType, productId, nameSnapshot, quantity, unitPrice, notes, currentUser }) => {
  const resolvedProduct = await resolveCommandItemProduct({ productType, productId, nameSnapshot });

  if (resolvedProduct.error) {
    return { error: resolvedProduct.error, statusCode: 400 };
  }

  const command = await findCommandById(commandId);
  if (!command) {
    return { error: 'Command not found', statusCode: 404 };
  }

  if (command.status !== COMMAND_STATUSES.OPEN) {
    return { error: 'Only open commands can receive new items', statusCode: 409 };
  }

  if (!canOperateOnCommand(currentUser, command)) {
    return { error: 'You do not have permission to update this command', statusCode: 403 };
  }
  ...
  appendAuditEntry(command, 'command_item_added', currentUser, {
...
const updateItemInCommand = async ({ commandId, itemId, quantity, unitPrice, notes, status, currentUser }) => {
  const command = await findCommandById(commandId);
  if (!command) {
    return { error: 'Command not found', statusCode: 404 };
  }
  ...
  if (command.status !== COMMAND_STATUSES.OPEN) {
    return { error: 'Only open commands can be updated', statusCode: 409 };
  }
  ...
  if (!item) {
    return { error: 'Command item not found', statusCode: 404 };
  }
  ...
  if (quantity !== undefined) {
    ...
    if (!Number.isInteger(normalizedQuantity) || normalizedQuantity < 1) {
      return { error: 'Quantity must be a valid integer greater than zero', statusCode: 400 };
    }
  ...
  if (unitPrice !== undefined) {
    ...
    if (normalizedUnitPrice === null) {
      return { error: 'Unit price must be a valid non-negative number', statusCode: 400 };
    }
  ...
  if (status !== undefined) {
    if (!Object.values(COMMAND_ITEM_STATUSES).includes(status)) {
      return { error: 'Invalid command item status', statusCode: 400 };
    }
    item.status = status;
  }

  appendAuditEntry(command, 'command_item_updated', currentUser, {
